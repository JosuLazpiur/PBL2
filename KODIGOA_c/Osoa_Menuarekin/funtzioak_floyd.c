...
void Floyd_Warshall_1(int F[N_MAX][N_MAX], int S[N_MAX][N_MAX], int tamaina_n) //FLOYD-WARSHALL PROGRAMA NAGUSIA
{
  int i, j, k;
...
void printf_matrizearen_pisua(int F[N_MAX][N_MAX], int tamaina_n) //MATRIZEAREN PISUA BISTARATU
{
  int i = 0, j = 0;
...
void printf_matrizearen_ibilbidea(int S[N_MAX][N_MAX], int tamaina_n) //MATRIZEAREN IBILBIDEA BISTARATU
{
  int i = 0, j = 0;
...
void eskatu_ibilbidea_1(int* A, int* B, int tamaina_n)
{
  ...
  while (egoera == 0)
  {
    printf("\n  Nondik norako ibilbidea egin nahi duzu? (A, B)\n\t> ");
    ...
    *B = b;
    ...
    {
      printf("[ERROREA] Emandako puntua ez dago matrizean.\n");
      egoera = 0;
    }
  }
}
...
void ibilbidea_motzena_kalkulatu_1(int S[N_MAX][N_MAX], int A, int B, int ibilbidea[], int tamaina_n)
{
  ...
  if (aux != B + 1)
  {
    ibilbidea_motzena_kalkulatu_1(S, A + 1, aux, ibilbidea, tamaina_n);
    ibilbidea_motzena_kalkulatu_1(S, aux, B + 1, ibilbidea, tamaina_n);
  }
  else
  {
...
void printf_ibilbidea_1(int ibilbidea[N_MAX]) //IBILBIDEA BISTARATU
{
  int amaitu = 0;
  int i = 0;
  printf("  IBILBIDEA: ");
...
void prestatu_nodoak_1(NODOA Nodoen_m[N_MAX][N_MAX], int Nodoak[N_MAX][N_MAX], int x, int y) //NODOAK PRESTATU FLOYD-WARSHALL-RAKO
{
  int i, j;
  for (i = 0; i < y; i++)
  {
...
void robota_funtzioak_1(int mapa[N_MAX][N_MAX], int* A, int* B, NODOA nodoen_lista[N_MAX * N_MAX], int tamaina_mapa_x, int tamaina_mapa_y, int tamaina_n, int ibilbidea[N_MAX], int S[N_MAX][N_MAX], NODOA Nodoen_m[N_MAX][N_MAX], int Nodoak[N_MAX][N_MAX], int ibilbidea_giroekin[N_MAX], int x, int y)
{
  ...
  bistaratu_mapa_1(mapa, nodoen_lista, tamaina_mapa_x, tamaina_mapa_y, tamaina_n, Nodoak);


  //IBILBIDEAK BBORRATU ETA -1 ERA HASIERATU
  for (i = 0; i < N_MAX; i++) ibilbidea[i] = -1;
  for (i = 0; i < N_MAX; i++) ibilbidea_giroekin[i] = -1;

  eskatu_ibilbidea_1(A, B, tamaina_n);

  ibilbidea[0] = *A;
  ibilbidea_motzena_kalkulatu_1(S, *A, *B, ibilbidea, tamaina_n);

  printf_ibilbidea_1(ibilbidea);

  printf_kotxearen_pausuak_1(Nodoen_m, Nodoak, ibilbidea, ibilbidea_giroekin, nodoen_lista);

  while (egoera == 0)
  {
    printf("\nAutoa martxan jartzea nahi duzu? (1/0)\n\t> ");
    fgets(str, MAX_STR, stdin);
    ...
    {
      egoera = 1;
      main_mugitu_1(ibilbidea_giroekin, ibilbidea);
    }
  }
}
...
void printf_kotxearen_pausuak_1(NODOA Nodoen_m[N_MAX][N_MAX], int Nodoak[N_MAX][N_MAX], int ibilbidea[N_MAX], int ibilbidea_giroekin[N_MAX], NODOA nodoen_lista[N_MAX * N_MAX])
{
  ...
  norabidea_galdetu_1(&kotxea_norabidea);

  printf("\n\n  KOTXEAK EGINDAKO PAUSOAK:\n\n");

  while (ibilbidea[i + 1] >= 0 && ibilbidea[i + 1] < INF) // PISU MATRIZEAREKIN KONPARATU
  {
    bilatu_matrizean_1(Nodoak, ibilbidea[i], &n, &m, nodoen_lista); //AURKITU POSIZIOA MATRIZEA
    bilatu_helmuga_1(Nodoen_m, ibilbidea[i + 1], n, m, &norabidea); // IKUSI IBILBIDEAREN HURRENGO NORAKOA
    if (norabidea == BEHERA)
    {
      switch (kotxea_norabidea)
      {
      case GORA: //BIRATU BI ALDIZ
        printf("\tEz biratu\n");
        atzera = 1;
        break;
      case BEHERA:
        ...
        break;
      case ESKUINERA:
        printf("\tEskuinera biratu\n");
        ibilbidea_giroekin[j] = ESKUINERA_GIRATU;
        j++;
        break;
      case EZKERRERA:
        printf("\tEzkerrera biratu\n");
...
void norabidea_galdetu_1(int* kotxea_norabidea)
{
  ...
  while (egoera != 1)
  {
    printf("\n\n  Noruntz begira hasiko da robota? \n\t0.- Beheruntz\n\t1.- Goruntz \n\t2.- Eskuineruntz \n\t3.- Ezkerreruntz\n  > ");
    fgets(str, MAX_STR, stdin);
...
void main_mugitu_1(int ibilbidea_giroekin[N_MAX], int ibilbidea[N_MAX])
{
  ...
  if (ibilbidea[i] == ibilbidea[i + 1])
  {
    ...
    hasieratu_1();
    gpioInitialise();
    eskuinera_tanke_buklea(0.1);
    ezkerrera_tanke_buklea(0.1);
    amaitu_1();
  }
  else
  {
    while (ibilbidea_giroekin[i] != AMAITU)
    {
      ...
      {
      case ATZERA: //atzera
        atzera_buklea(t_aurrera);
        break;
      case AURRERA: //aurrera
        aurrera_buklea(t_aurrera);
        break;
      case ESKUINERA: //eskuinera
        eskuinera_tanke_buklea(t_giro);
        break;
      case EZKERRERA: //ezkerrera
        ezkerrera_tanke_buklea(t_giro);
        break;
      default:
        break;
      }
...
void irakurri_mapa_1(char fitx_izena[], int mapa[N_MAX][N_MAX], int* tamaina_mapa_x, int* tamaina_mapa_y, NODOA nodoen_lista[N_MAX * N_MAX])
{
  ...
  if (fitx == NULL)
  {
    printf("[ERROREA] Ezin izan da fitxategia zabaldu.\n");
    return;
  }
  ...
  while (fscanf(fitx, "%c", &n) == 1)
  {
    if (n == '\n')
    {
      while (fila == 0 && i < hilara && batura <= 1) //LEHENENGO ILARA NULUA BADA KENDU
      {
        batura += mapa[0][i];
        i++;
      }
    ...
    else if (n == 32); // HUTSUNEAK EZ HARTU KONTUAN
    else
    {
...
void nodoak_funtzioak_1(int mapa[N_MAX][N_MAX], int F[N_MAX][N_MAX], int Nodoak[N_MAX][N_MAX], int* tamaina_n, int* x, int* y, NODOA Nodoen_m[N_MAX][N_MAX], int tamaina_mapa_x, int tamaina_mapa_y, NODOA nodoen_lista[N_MAX * N_MAX]) //tamaina_n gaizki dago, nodoen_kop/2 da, beraz aurreko funtziotik ezin da lortu
{
  int nodo_kop = 0;

  nodoak_aurkitu_1(mapa, nodoen_lista, tamaina_mapa_x, tamaina_mapa_y, &nodo_kop);

  nodoen_bizilagunak_aurkitu_1(nodoen_lista);

  nodoen_matrizea_sortu_1(nodoen_lista, Nodoen_m, nodo_kop, tamaina_n, Nodoak, tamaina_mapa_x, tamaina_mapa_y);

  F_kalkulatu_nodoekin_1(nodoen_lista, *tamaina_n, F);
}
...
void nodoak_aurkitu_1(int mapa[N_MAX][N_MAX], NODOA nodoen_lista[N_MAX], int tamaina_mapa_x, int tamaina_mapa_y, int* nodo_kop)
{
  ...
  int i = 0, j = 0;
  int irteera_kop = 0;
  int nodoa = 0; //NODOA AURKITZEN BADU 1-ERA
  int kop = 0;

  while (amaitu == 0)
  {
    irteera_kop = 0;
    if (mapa[i][j] == 1) //IKUSI EA NODOA DEN
    {
...
void galdetu_irakurri_beharrekoa_1(char fitx_izena[MAX_STR])
{
  ...
  while (amaitu == 0)
  {
    ...
    printf("\n  Zein fitxategietako mapa erabili nahi duzu?\n\t> ");
    ...
    {
      printf("[ERROREA] Ez da fitxategia irakurri.");
      getchar();
    }
  }
}
...
void menua_floyd_1(int mapa[N_MAX][N_MAX], NODOA nodoen_lista[N_MAX], int tamaina_mapa_x, int tamaina_mapa_y, int tamaina_n, int F[N_MAX][N_MAX], int S[N_MAX][N_MAX], int ibilbidea[N_MAX], int* A, int* B, NODOA Nodoen_m[N_MAX][N_MAX], int Nodoak[N_MAX][N_MAX], int ibilbidea_giroekin[N_MAX], int x, int y)
{
  ...
  while (amaitu == 0)
  {
    ...
    printf("\n\n========================== MENUA FLOYD ==============================================");
    printf("\n\n\t1. MAPA bistaratu");
    printf("\n\n\t2. PISU eta IBILBIDE matrizeak bistaratu.");
    printf("\n\n\t3. Ibilbide laburrena kalkulatu eta exekutatu.");
    printf("\n\n\t0. irten.");
    printf("\n\n  Zer egin nahi duzu?\n\t> ");
    fgets(str, MAX_STR, stdin);
    sscanf(str, "%d", &aukera);
    ...
    case 2:
      system("clear");
      printf_matrizearen_pisua(F, tamaina_n);
      printf_matrizearen_ibilbidea(S, tamaina_n);
      getchar();
      break;
    case 3:
      robota_funtzioak_1(mapa, A, B, nodoen_lista, tamaina_mapa_x, tamaina_mapa_y, tamaina_n, ibilbidea, S, Nodoen_m, Nodoak, ibilbidea_giroekin, x, y);
      break;
    case 0:
      amaitu = 1;
      break;
